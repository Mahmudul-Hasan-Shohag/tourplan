...
<header>
  ...
  <div class="navbar navbar-dark bg-dark shadow-sm">
    <div class="container">
      ...
      <a href="{{route('planner.spot.form')}}" class="navbar-brand d-flex align-items-center">
      ...
      @foreach($spot as $data)
        <div class="col">
          <div class="card shadow-sm">
            <img width="300px" height="300px" src="{{url('/uploads/planner',$data->image)}}" alt="">

            <div class="card-body">
              <p class="card-text">{{$data->spot_name}}</p>
              <p class="card-text">Date: {{$data->journey_date}}</p>
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  
                  <a href="{{route('planner.spot.details',$data->id)}}" class="btn btn-info">Show Details</a>
                </div>
                <small class="text-muted"><p style="color:crimson">Total Cost: {{$data->total_cost}}</p></small>
              </div>
            </div>
          </div>
        </div>
